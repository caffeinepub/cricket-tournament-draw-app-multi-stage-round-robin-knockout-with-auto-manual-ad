import { RoundRobinRoundConfig, KnockoutStage, StageAdvancementConfig } from './types';
...
export function getStageAdvancementLabel(stageNumber: number, configs: StageAdvancementConfig[]): string {
  const config = configs.find(c => c.stageNumber === stageNumber);
...
function formatDestination(dest: StageAdvancementConfig['winnerDestination']): string {
  if (dest.type === 'NextStage') {
    return `Round ${dest.stageIndex + 1}`;
  } else {
    return dest.entryPoint.replace('finals', '-Finals');
  }
}
